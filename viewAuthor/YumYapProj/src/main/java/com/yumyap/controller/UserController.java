...
public class UserController {
	...
	public ResponseEntity<UserDto> loadProfile(@RequestBody UserDto userDto) {
		...
		logger.trace("Loading Profile (userDto="+userDto);
		return new ResponseEntity<UserDto>(userDto, HttpStatus.OK);
	}
	
	@RequestMapping(value = "/viewAuthor", method = { RequestMethod.GET }, consumes ={
			MediaType.APPLICATION_JSON_VALUE }, produces = { MediaType.APPLICATION_JSON_VALUE })
	public ResponseEntity<UserDto> loadProfile(@RequestBody SimpleUserDto simpleDto) {
		logger.trace("viewAuthor (simpleDto="+simpleDto);
		return new ResponseEntity<UserDto>(userService.getAuthor(simpleDto), HttpStatus.OK);
	}
